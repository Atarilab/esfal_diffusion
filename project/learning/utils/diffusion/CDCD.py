...
class CDCD(DDPM):
    ...
    def _compute_loss(self,
        ...
        if self.gamma_scale_loss_snr > 0.:
            criterion.reduction = "none"
            loss = criterion(logits, index)
            snr = self.noise_scheduler.snr(timesteps)
            base_weight_max = (torch.stack([snr, self.gamma_scale_loss_snr * torch.ones_like(timesteps)], dim=1).min(dim=1)[0] / snr)
            loss = torch.mean(loss, dim=-1) * base_weight_max
            loss = torch.mean(loss)
