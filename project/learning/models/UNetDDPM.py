...
class ConditionalUnet1DBase(nn.Module):
    ...
    def forward(self,
        ...
        # (B,C,T)
        global_cond = global_cond.reshape(-1, self.global_cond_dim)
...
class AttentionConditionalUnet1D(ConditionalUnet1DBase):
    def __init__(self,
        input_dim,
        embedding_dim,
        diffusion_step_embed_dim=256,
    ...
    def forward(self,
                ...
                self_conditioning: torch.Tensor=None,
                ):
        """
        noisy_data_samples: (B,T,input_dim)
        timestep: (B,) or int, diffusion step
        ...
        if global_cond != None:
            ...
            if self.exclude_first > 0:
                ...
                global_cond = self.norm(out + query)
