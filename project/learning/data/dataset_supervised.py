...
class JumpDataset(Dataset):
    ...
    def load_data(self):
        ...
        data = {
            'states': torch.tensor(states, dtype=torch.float32).reshape(N_samples, -1),
    ...
    def __getitem__(self, idx):
        join_pos = self.data['states'][idx, 7:7+12]
        ...
        target_locations = self.data['target_contacts'][idx]

        state_goal_conditionning = torch.cat((goal_contact_base, feet_contact_base, velocities, join_pos, stones_pos_base))

        batch = {
            "target": target_locations,
            "input": state_goal_conditionning,
        }
        
        return batch

def shuffle_collate(batch):
    condition = torch.stack([d["input"] for d in batch], dim=0)
    data = torch.stack([d["target"] for d in batch], dim=0)
    B, L = data.shape
    data = data.reshape(B, -1, 3)
    ...
    shuffled_batch = {
        "target" : data.reshape(B, -1),
        "input" : shuffled_condition,
    }
    
    return shuffled_batch

def get_dataloaders(data_dir, dataset, batch_size, return_index:bool=False, augmentation:bool=False, shuffle:bool=True, noise_level:float=1.e-3, train_only=False):
